...
global_all_names = ['吴宣仪', '孟美岐', 'Yamy', '周洁琼', '鞠婧祎', '李艺彤', '于文文', '陈意涵', '王菊', '苏诗丁'];
    ...
    ajax.onreadystatechange = function(){
        if(ajax.readyState == 4 && ajax.status == 200){
            ...
            if(response_json["ok"] > 0 && response_json.data.length > 0)
            {
                ...
                multi_series_inc_data = response_json;
...
function build_inc_chart(singers_data, filters)
{
    ...
    var legend = svg.selectAll(".legend")
        .data(multi_series_inc_data.data)
        .enter()
        .append("g")
        .attr("class", "legend")
        .attr("transform", function(d, i){
            var legendx = i*90 + margin.left;
            var legendy = 20;
    ...
    function AddDotsGroupForSinger(singer_item)
    {
        ...
        dots_group.selectAll("circle")
            ...
            .on("mouseover", function(d){
                ...
                var current_y = d[2];
                var start_item = singer_item["list"][0]
                var end_item = singer_item["list"][singer_item["list"].length-1];
                start_item[2] = current_y;
                end_item[2] = current_y;
                bottom_layer_g.append("path")
                    // .attr("transform", "translate(" + margin.left + "," + margin.top + ")")
                    .attr("class", "tip-h-line")
                    .attr("id", "tip-h-line-" + singer_item["name"])
                    .attr("d", function(){ return line([start_item, end_item]); })
            ...
            .on("mouseout", function(d){
                ...
                // TODO: remove line
                d3.select("#tip-h-line-" + singer_item["name"]).remove()
...
function bind_checkbox_listener()
{
    ...
    for(var i=0; i<checkboxes.length; i++)
    {
        ...
        if(global_name_filter.indexOf(checkboxes[i].value)>-1)
        {
